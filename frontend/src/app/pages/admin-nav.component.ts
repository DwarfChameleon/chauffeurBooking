...
type AdminTab = 'dashboard' | 'users' | 'drivers' | 'bookings' | 'verification';
...
.nav-item { display:flex; flex-direction:column; align-items:center; justify-content:center; gap:5px; min-width:86px; min-height:54px; border:0; border-radius:8px; background:transparent; color:var(--admin-muted,#6b7280); font:inherit; font-size:10px; font-weight:800; cursor:pointer; }
.nav-item ion-icon { font-size:21px; }
.nav-item.active { color:#1954d1; background:rgba(25,84,209,.1); }
...
:host-context(body.dark-theme) .nav-item.active { color:#d8b4fe; background:rgba(124,58,237,.18); }
