...
<ion-content>
  ...
  `,
  styles: [`
:host { display:block; --ink:#172033; --muted:#667085; --line:#e6ebf2; --blue:#1954d1; }
...
.eyebrow { margin:0 0 7px; color:#5571a7; font-size:11px; font-weight:900; letter-spacing:.12em; text-transform:uppercase; }
...
.role-card.active { border-color:var(--blue); box-shadow:0 0 0 3px rgba(25,84,209,.12); }
.role-card ion-icon { color:var(--blue); font-size:28px; }
...
.account-type-button { display:grid; justify-items:start; gap:8px; min-height:132px; padding:16px; border:1px solid rgba(25,84,209,.25); border-radius:8px; color:var(--ink); background:transparent; text-align:left; cursor:pointer; }
.account-type-button.active { border-color:var(--blue); background:rgba(25,84,209,.06); box-shadow:0 0 0 3px rgba(25,84,209,.1); }
.account-type-button ion-icon { color:var(--blue); font-size:26px; }
.account-type-button strong { font-size:15px; }
.account-type-button span { color:var(--muted); font-size:13px; line-height:1.45; }
.back-link { display:inline-flex; align-items:center; gap:5px; margin-bottom:10px; border:0; color:var(--blue); background:transparent; font:inherit; font-size:12px; font-weight:900; cursor:pointer; }
...
export class RegisterPage {
  ...
  async submit() {
    ...
    try {
      ...
      const session = await this.auth.register({ name: this.name, contact, method, email: this.email.trim(), phone: this.phone.trim(), password: this.password, role: this.role, employerType: this.role === 'user' ? this.employerType : undefined, location: JSON.stringify(location) });
